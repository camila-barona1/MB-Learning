...
if (!isset($_SESSION['stuLoginEmail'])){
    echo "<script> location.href = 'loginarsingup.php' </script>";
}else{
    $stuEmail = $_SESSION['stuLoginEmail'];

    if (isset($_REQUEST['payLessonBtn'])) {
      if (isset($_POST['ORDER_ID']) && isset($_POST['Amount'])) {
          ...
          INSERT INTO
              courseorder 
              (order_id, stu_email, courser_id, status, amount, order_date)
          VALUES
              ('$order_id','$stuEmail','$course_id','$status','$amount','$date')    
          ";
  
          if ($conn->query($sql) == true) {
              echo "Redireccionando a mi perfil...";
              echo "<script> setTimeout(() =>{
                window.location.href = './Students/myCourses.php';
              }, 1500) </script>";
          }
      }
...
<body>
<div class='subject'>MBSchool </br><strong>Credit Card Checkout</strong></div>
...
<div class='checkout'>
  <div class='order'>
    ...
    <img width="200px" src='<?php echo (isset($_POST['course_img'])) ? $_POST['course_img'] : '' ?>'>
      ...
      <h4>Duracion: <?php echo (isset($_POST['course_duration'])) ? $_POST['course_duration'] : '' ?></h4>
    <br>
    <br>
    <h5 class='total'>Total</h5><h1>$ <?php echo (isset($_POST['amount'])) ? $_POST['amount'] : '' ?></h1>
  ...
  <div id='payment' class='payment'>
    ...
    <div class='card-form'>
      ...
      <form action="" method="POST">
            <input id="ORDER_ID" type="hidden" name="ORDER_ID" value="<?php echo  "ORD" . rand(10000,99999999)?>" readonly>
            <input id="EMAIL_ORDER" type="hidden" name="EMAIL_ORDER" value="<?php if (isset($stuEmail)) {
                echo $stuEmail;
            } ?>" readonly>
            <input type="hidden" name="Amount" value="<?php if (isset($_POST['amount'])) {
                echo $_POST['amount'];
            } ?>" readonly>

            <input type="hidden" name="producto" value="<?php if (isset($_POST['course_name'])) {
